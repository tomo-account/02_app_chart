...
with col_stock:
    watch_a_label = f"List A（{len(WATCH_A)}）"
    watch_b_label = f"List B（{len(WATCH_B)}）"
    watch_c_label = f"List C（{len(WATCH_C)}）"
    watch_d_label = f"List D（{len(WATCH_D)}）"
    watch_e_label = f"List E（{len(WATCH_E)}）"
    stock_mode = st.radio(
        "銘柄選択",
...
with col_code:
    ...
    if is_manual:
        ...
        ticker_list = [t.replace(".T", "") for t in WATCH_D]
    elif stock_mode == watch_e_label:
        ticker_list = [t.replace(".T", "") for t in WATCH_E]
    else:
        ticker_list = []
with col_days:
    end_date    = st.date_input("2. 基準日", value=datetime.now())
    period_days = st.number_input("3. 遡る日数", min_value=1, max_value=60, value=10, step=1)
with col_sort:
