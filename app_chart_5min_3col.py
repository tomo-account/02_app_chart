...
def create_5min_line_chart(df):
    if df.empty:
        return alt.Chart(pd.DataFrame({'x': [], 'y': []})).mark_line().encode()
    ...
    line = alt.Chart(df_c).mark_line(color='#FF4B4B', strokeWidth=1.5).encode(
        x=alt.X('x_key:O', sort=None, title=None, axis=alt.Axis(labels=False, grid=False)),
        y=alt.Y('Close:Q', title=None, scale=alt.Scale(zero=False), axis=alt.Axis(minExtent=Y_AXIS_WIDTH)),
    )
    return alt.layer(am_rules, pm_rules, line).properties(height=180).configure_view(strokeOpacity=0)


def create_5min_pct_change_chart(df, y_domain):
    if df.empty:
        return alt.Chart(pd.DataFrame({'x': [], 'y': []})).mark_area().encode()
    pm_start_time = time(12, 30)
    df_c = df.copy().sort_values('Datetime')
    df_c['x_key'] = df_c['Datetime'].dt.strftime('%y/%m/%d %H:%M')
    df_c['date']  = df_c['_date'] if '_date' in df_c.columns else df_c['Datetime'].dt.date
    ...
    df_c['is_am_line'] = am_mask
    df_c['is_pm_line'] = (df_c['Datetime'].dt.time == pm_start_time)
    first_close = df_c['Close'].iat[0]
    df_c = df_c.assign(PctChange=(df_c['Close'] - first_close) / first_close * 100)
    am_rules = alt.Chart(df_c[df_c['is_am_line']]).mark_rule(color='#CCCCCC').encode(x='x_key:O')
    pm_rules = alt.Chart(df_c[df_c['is_pm_line']]).mark_rule(color='#EEEEEE').encode(x='x_key:O')
    gradient_fill = alt.Gradient(
        gradient='linear',
    ...
    return alt.layer(am_rules, pm_rules, area).properties(height=180).configure_view(strokeOpacity=0)
...
# --- Streamlit UI ---
# ================================================
st.set_page_config(page_title="銘柄比較チャート", page_icon="📈")
...
with st.sidebar:
    ...
    period_days = st.number_input("5分足日数", min_value=1, max_value=60, value=3, step=1)
    pct_range   = st.number_input("騰落率Y軸幅", min_value=0.5, max_value=20.0, value=3.0, step=0.5)
    daily_months = 6  # 固定値
    ...
    # --- チャート選択 ---
    st.markdown("### チャート選択")
    show_5m    = st.checkbox("5分足ローソクチャート", value=False)
    show_daily = st.checkbox("5分足ラインチャート", value=True)
    show_pct   = st.checkbox("5分足騰落率チャート", value=False)
...
if cols_per_stock == 0:
    ...
    st.stop()
stocks_per_row = 3 // cols_per_stock
...
def render_row(row_items, cols, show_daily, show_pct, show_5m, cols_per_stock, common_y_domain):
    ...
    for row_idx, item in enumerate(row_items):
        ...
        ticker_display = str(item['ticker']).removesuffix('.T')
        name_text = f"**{item['name']}** ({ticker_display})"
        if not item["df"].empty:
            hi = item["df"]['High'].max()
            lo = item["df"]['Low'].min()
            metrics_text = f"Max {hi:,.1f}  Min {lo:,.1f}"
        else:
            metrics_text = ""
        current_slot = 0
        if show_5m:
        ...
        if show_daily:
            with cols[base_col + current_slot]:
                st.write(name_text if current_slot == 0 else "&nbsp;")
                if not item["df"].empty:
                    st.altair_chart(create_5min_line_chart(item["df"]), width='stretch')
                else:
                    st.warning("5分足データなし(60日制限)")
                if current_slot == cols_per_stock - 1:
                    st.caption(metrics_text)
        ...
        if show_pct:
            with cols[base_col + current_slot]:
                st.write(name_text if current_slot == 0 else "&nbsp;")
                if not item["df"].empty:
                    st.altair_chart(create_5min_pct_change_chart(item["df"], common_y_domain), width='stretch')
                else:
                    st.warning("5分足データなし(60日制限)")
                if current_slot == cols_per_stock - 1:
                    st.caption(metrics_text)
...
if show_pct:
    ...
    for item in scored_tickers:
        if not item["df"].empty:
            fc = item["df"]['Close'].iat[0]
            all_pct_values.extend(((item["df"]['Close'] - fc) / fc * 100).tolist())
    if all_pct_values:
        import numpy as np
        median_pct = float(np.median(all_pct_values))
        common_y_domain = [median_pct - pct_range, median_pct + pct_range]
    else:
        common_y_domain = [-pct_range, pct_range]
